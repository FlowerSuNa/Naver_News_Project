...
# end if
                    ...
                    test<-test[-grep("img",test)]
                    test<-test[grep("Î©îÏù∏?â¥?ä§",test):grep("?â¥?ä§Î¶¨Ïä§?ä∏",test)[2]]
...
# end if
                            ...
                            if(ncnt==3)
                            ...
                            else
                                {
                                    ...
                                    postTime<-tst[grep("Í∏∞ÏÇ¨?ûÖ?†•",tst)]
                                    chgTime<-tst[grep("ÏµúÏ¢Ö?àò?†ï",tst)]
                                    if(length(grep("Î≥∏Î¨∏ ?Ç¥?ö©",tst))>1)
                                        {
                                            con<-tst[grep("Î≥∏Î¨∏ ?Ç¥?ö©",tst)[1]:grep("Î≥∏Î¨∏ ?Ç¥?ö©",tst)[2]]
                                        }
                                    else
                                        {
                                            con<-tst[grep("[(Í∞Ä-?û£?Ñ±-?Öé?Öè-?Ö£)]",tst)[ grep("[(Í∞Ä-?û£?Ñ±-?Öé?Öè-?Ö£)]",tst)>grep("Î≥∏Î¨∏ ?Ç¥?ö©",tst)][1]]
                                        }
# end if

                                    cate<-gsub("[^(Í∞Ä-?û£?Ñ±-?Öé?Öè-?Ö£)]","",cate)
                                    title<-gsub('<meta property=\"og:title\"\t\t\tcontent=\"',"",title)
                                    title<-gsub('\"/>',"",title)
                                    author<-gsub('<meta property=\"og:article:author\"\tcontent=\"',"",author)
                                    author<-gsub('\"/>',"",author)
                                    author<-gsub(" ?Ñ§?ù¥Î≤? ?â¥?ä§","",author)
                                    author<-gsub("[[:punct:]]", "", author)
                                    author<-gsub(" ", "", author)
                                    postTime<-gsub("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>","",postTime)
                                    postTime<-gsub("Í∏∞ÏÇ¨?ûÖ?†• ","",postTime)
                                    if (length(chgTime)!=0)
                                        {
                                            chgTime<-gsub("\t","",chgTime)
                                            chgTime<-gsub("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>","",chgTime)
                                            chgTime<-gsub("ÏµúÏ¢Ö?àò?†ï ","",chgTime)
                                        }
                                    else
                                        {
                                            chgTime<-postTime
...
# end if
                            ...
                            if(ncnt==3)
                            ...
                            else
                                {
                                    ...
                                    title<-tst[grep("og:title",tst)]
                                    author<-tst[grep(":author\"",tst)]
                                    postTime<-tst[grep("Í∏∞ÏÇ¨?ûÖ?†•",tst)+1]
                                    chgTime<-tst[grep("ÏµúÏ¢Ö?àò?†ï",tst)]
                                    if(length(grep("Í∏∞ÏÇ¨ ?Ç¥?ö©",tst))>1)
                                        {
                                            con<-tst[grep("Í∏∞ÏÇ¨ ?Ç¥?ö©",tst)[1]:grep("Í∏∞ÏÇ¨ ?Ç¥?ö©",tst)[2]]
                                        }
                                    else
                                        {
                                            con<-tst[grep("[(Í∞Ä-?û£?Ñ±-?Öé?Öè-?Ö£)]",tst)[ grep("[(Í∞Ä-?û£?Ñ±-?Öé?Öè-?Ö£)]",tst)>grep("Í∏∞ÏÇ¨ ?Ç¥?ö©",tst)][1]]
                                        }
# end if
                                    ...
                                    author<-gsub('<meta property=\"og:article:author\" content=\"',"",author)
                                    author<-gsub('\"/>',"",author)
                                    author<-gsub("?Ñ§?ù¥Î≤? ?ä§?è¨Ï∏? ","",author)
                                    author<-gsub("[[:punct:]]", "", author)
                                    author<-gsub(" ", "", author)
                                    postTime<-gsub("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>","",postTime)
...
# end if
                                            ...
                                            author<-gsub('<meta property=\"og:article:author\"\tcontent=\"',"",author)
                                            author<-gsub('\"/>',"",author)
                                            author<-gsub("?Ñ§?ù¥Î≤ÑTV?ó∞?òà","",author)
                                            author<-gsub("[[:punct:]]", "", author)
                                            author<-gsub(" ", "", author)
